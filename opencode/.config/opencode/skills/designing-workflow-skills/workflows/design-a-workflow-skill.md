...
1. **For each action in each phase**, identify the tool it needs:
   ...
   - User confirmation -> question/AskUserQuestion (platform-specific)
   - Delegating work -> task/Task (platform-specific)
   - Tracking steps -> todowrite/TodoWrite (platform-specific)
