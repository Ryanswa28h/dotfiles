...
description: >-
  Guides the design and structuring of workflow-based skills
...
Most skills and agents should include progress tracking tools — these enable progress tracking during multi-step execution and are useful even for skills that don't explicitly manage tasks.
</principle>
...
- Non-skill development — this is specifically for skill architecture
...
| **Task-Driven** | Complex dependencies, partial failure tolerance | Task tracking with dependency declarations |
...
|---------------|---------------|
| Read-only analysis skill | Read, Glob, Grep |
| Interactive analysis skill | Read, Glob, Grep, question |
| Code generation skill | Read, Glob, Grep, Write, Bash |
| Pipeline skill | Read, Write, Glob, Grep, Bash, question, task, todowrite |
| Read-only agent | Read, Grep, Glob |
| Action agent | Read, Grep, Glob, Write, Bash |
