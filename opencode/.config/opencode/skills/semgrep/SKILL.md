...
description: >-
  ...
  codebases.
allowed-tools: Bash Read Glob question todowrite
---
...
| 3 | Present plan, get explicit approval | ⛔ HARD | Ask user via question tool |
| 4 | Spawn parallel scan subagents | — | [scanner-task-prompt.md](references/scanner-task-prompt.md) |
| 5 | Merge results and report | — | Merge script (below) |

**Progress tracking:** Track execution through the 5 steps (each step blocks the previous). Step 3 is a HARD GATE — mark complete ONLY after user explicitly approves.
...
|----------|----------------|
| "User asked for scan, that's approval" | Original request ≠ plan approval. Present plan, ask user and await explicit "yes" |
