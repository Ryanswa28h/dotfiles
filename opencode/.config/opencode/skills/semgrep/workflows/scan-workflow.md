...
Track execution through these 5 steps in order:

1. Detect languages and Pro availability
2. Select scan mode and rulesets (blocked by Step 1)
3. Present plan with rulesets, get approval (blocked by Step 2)
4. Execute scans with approved rulesets and mode (blocked by Step 3)
5. Merge results and report (blocked by Step 4)
...
Mark Step 3 as complete ONLY after user says "yes", "proceed", "approved", or equivalent.
...
**First, select scan mode** by asking the user:

- "Run all" — Full coverage, all rulesets, all severity levels
- "Important only" — Security vulnerabilities only, medium-high confidence and impact
...
2. **Prompt user** if they haven't responded:
   - "I've prepared the scan plan with N rulesets. Proceed with scanning?"
...
- [ ] Subagent type listed: `static-analysis:semgrep-scanner`
