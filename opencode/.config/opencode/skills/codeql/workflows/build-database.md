...
1. Detect language and configure
2. Build database (blocked by Step 1)
3. Apply fixes if needed (blocked by Step 2)
4. Assess quality (blocked by Step 3)
5. Improve quality if needed (blocked by Step 4)
6. Generate final report (blocked by Step 5)
...
**Failure (all methods exhausted):** Ask user:
- "Accept current state" (if any database exists)
- "I'll fix the build manually and retry"
- "Abort"
