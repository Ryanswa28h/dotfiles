...
1. Select database and detect language
2. Select scan mode, check additional packs (blocked by Step 1)
3. Select query packs, model packs, and threat models (blocked by Step 2)
4. Execute analysis (blocked by Step 3)
5. Process and report results (blocked by Step 4)

### Gates

| Step | Gate Type | Cannot Proceed Until |
|------|-----------|---------------------|
...
if [ -z "$DB_NAME" ]; then
  ...
  if [ ${#FOUND_DBS[@]} -eq 0 ]; then
    ...
    # Multiple databases found — present to user with each DB's path and language
...
**When multiple databases are found**, ask the user to select — list each database with its path and language. **Skip the selection prompt if the user already specified which database to use in their prompt.**
...
- "Run all" — Maximum coverage, all queries from all installed packs
- "Important only" — Security vulnerabilities only, medium-high precision
...
**Run-all mode:** Ask user to confirm "Use all" or "Select individually". Always ask — the user needs to see which packs will run.

#### 3b: Select Model Packs (if any detected)

**Skip if no model packs detected in Step 2c.**

Ask user: "Use all" / "Select individually" / "Skip".
...
**Always ask.** Do not default to "remote only" without user confirmation. Present options:

- "Remote only" — Default, HTTP requests, network input
- "Remote + Local" — Add CLI args, local files
- "All sources" — Remote, local, environment, database, file
- "Custom" — Select specific threat models individually
