...
allowed-tools: Bash Read Write Edit Glob Grep question todowrite
---
...
Then ask the user to select which database to use, or to build a new one. **Skip the selection prompt if the user explicitly stated which database to use or to build a new one in their prompt.**
...
**If user explicitly specifies** what to do (e.g., "build a database", "run analysis on ./my-db"), execute that workflow directly. **Do NOT prompt for database selection if the user's prompt already makes their intent clear** — e.g., "build a new database", "analyze the codeql database in static_analysis_codeql_2", "run a full scan from scratch".
...
| Multiple databases found | Ask user: list all with metadata, let user pick one or build new |
| User explicitly stated intent | Skip the selection prompt, act on their instructions directly |

### Database Selection Prompt

When existing databases are found **and the user did not explicitly specify which to use**, present options to the user. Show each database with language and creation time, plus a "build new" option:

```
Existing CodeQL Databases:
- <db_path_1> (language: python, created: 2026-02-24) — Reuse
- <db_path_2> (language: cpp, created: 2026-02-23) — Reuse
- Build a new database (fresh analysis)
```
