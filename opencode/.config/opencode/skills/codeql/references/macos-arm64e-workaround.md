...
- "Use build-mode=none" — Source-level analysis only. Misses some interprocedural data flow but catches most C/C++ vulnerabilities.
- "Install arm64 tools and retry" — Run `brew install llvm make`, then retry with Homebrew toolchain
- "Install Rosetta and retry" — Run `softwareupdate --install-rosetta`, then retry under x86_64 emulation
- "Abort" — Stop database creation
