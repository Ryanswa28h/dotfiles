...
*Use for Claude Code, OpenCode, Devin, AutoGPT, and any AI that takes autonomous actions. Runaway loops and scope explosion are the biggest credit killers in agentic workflows — stop conditions are not optional.*
...
**Claude Code / OpenCode — add Session Strategy block when relevant:**
```
## Session Strategy
[Pick one:]
...
- Compact first — Claude Code: run /compact [focus on X]. OpenCode: start a fresh session, no compact needed
```
