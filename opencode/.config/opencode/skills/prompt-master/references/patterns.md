...
| 37 | **Context rot on long sessions** | Keeps correcting in the same session for 60+ turns | New task = new session. For Claude Code: use /rewind instead of correcting, /compact at ~50% context. For OpenCode: start a fresh session — no /rewind or /compact available. Subagents for file-heavy investigation. |
