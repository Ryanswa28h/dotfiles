...
name: prompt-master
version: 1.7.0
description: Generates optimized prompts for AI tools, including OpenCode. Activates only when the user explicitly asks to write, fix, improve, or adapt a prompt for a specific AI tool (OpenCode, LLM, Cursor, Midjourney, image AI, video AI, coding agents, etc.). Does not activate for general conversation, coding tasks, document writing, or other non-prompt-engineering work.
---
...
**OpenCode**
- Agentic CLI tool — runs commands (bash), reads/writes files, edits code, searches codebase, delegates subagents, fetches web content, asks questions
- Starting state + target state + allowed/forbidden actions + scope locks + stop conditions
- Powered by DeepSeek or other LLMs — prompting style should account for the underlying model's strengths
- Always specify: exact file paths, what to change, what NOT to touch, and the done condition
- Session hygiene: new task = new session. OpenCode has no `/rewind` or `/compact` — start fresh conversations for independent tasks
- Subagents via the `task` tool — explicitly request for file-heavy research, verification, or parallel work: "Use a subagent to investigate X in parallel so it stays out of main context"
...
For prompts targeting agentic tools (Claude Code, OpenCode, Devin, Cursor, Windsurf, Cline, Bolt, SWE-agent, Manus, or anything that executes commands or edits files — mandatory for Templates G, H, M and any prompt referencing filesystem, terminal, dependency, or database operations), append this notice:
