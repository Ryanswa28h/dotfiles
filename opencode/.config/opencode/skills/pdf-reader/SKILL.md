...
All scripts use a venv at `skills/pdf-reader/scripts/.venv` with `pymupdf` installed. If the venv is missing, create it from `requirements.txt`:

```bash
python3 -m venv skills/pdf-reader/scripts/.venv
skills/pdf-reader/scripts/.venv/bin/pip install -r skills/pdf-reader/requirements.txt
```

**Python command:** Always invoke scripts with:
```
skills/pdf-reader/scripts/.venv/bin/python skills/pdf-reader/scripts/<script>.py [args]
```

## Scripts

All scripts are in `skills/pdf-reader/scripts/`.
...
| `pdf_render.py <path> [--pages SPEC] [--dpi N]` | Render pages to PNG images in `/tmp/opencode-pdf-*/` | `--pages`, `--dpi` (default 150) |
...
Run `python skills/pdf-reader/scripts/pdf_info.py <path>` on every new PDF before doing anything else. This tells you:
...
#### Short PDFs (≤15 pages)
- Extract all text: `python skills/pdf-reader/scripts/pdf_extract.py <path>`
- Render all pages: `python skills/pdf-reader/scripts/pdf_render.py <path>`
...
- Extract all text first: `python skills/pdf-reader/scripts/pdf_extract.py <path>` (cheap, gives structural overview)
- Check `python skills/pdf-reader/scripts/pdf_info.py <path>` output for pages with high `math_density` (>0.02) or `image_count` > 0 or low `text_length` (<100, likely diagram-only pages)
...
- For targeted questions: use `python skills/pdf-reader/scripts/pdf_search.py` to find relevant pages, then render those
...
1. `python skills/pdf-reader/scripts/pdf_search.py <path> "theorem 3.2"` — find the page
2. `python skills/pdf-reader/scripts/pdf_render.py <path> --pages <page>` — render just that page
...
```
1. python skills/pdf-reader/scripts/pdf_info.py <path> → assess size and content
...
```
1. python skills/pdf-reader/scripts/pdf_search.py <path> "theorem X" → find the page
2. python skills/pdf-reader/scripts/pdf_render.py <path> --pages <page> → render
3. Read the image, state the theorem precisely
```

### "Explain the proof on page N"
```
1. python skills/pdf-reader/scripts/pdf_render.py <path> --pages N → render the page
...
```
1. python skills/pdf-reader/scripts/pdf_info.py <path> → get TOC and page count
2. python skills/pdf-reader/scripts/pdf_extract.py <path> → full text
