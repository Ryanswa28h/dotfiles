...
1. **`pi agents` command** (primary) — run `pi agents` to get all agents known to the CLI, including user agents, plugin agents (e.g. `ecc:architect`), and built-in agents. This automatically covers ECC marketplace installs without any path configuration.
2. **File glob** (fallback, for reading agent content) — agent markdown files are read from:
   ...
   - `/home/ryan/.pi/agent/skills/herdr-pi-subagents/agents/**/*.md` + `/home/ryan/.pi/agent/skills/herdr-pi-subagents/agents/*.md` — global user agents
...
Run `pi agents` to get the full agent list. Parse each line:
...
- **User agents** have no prefix. Read the corresponding markdown file from `/home/ryan/.pi/agent/skills/herdr-pi-subagents/agents/` or `./agents/` to extract the name and description.
...
If no agents are found after running `pi agents` and probing file locations, inform the user: "No agents found. Run `pi agents` to verify your setup." Then stop.
...
- **Parallel Agent calls, not TeamCreate.** This skill uses parallel Agent tool calls for independent work. TeamCreate (a Pi tool for multi-agent dialogue) is only needed when agents must debate or respond to each other.
