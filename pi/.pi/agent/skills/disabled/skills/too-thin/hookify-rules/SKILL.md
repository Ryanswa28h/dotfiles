...
Hookify rules are markdown files with YAML frontmatter that define patterns to watch for and messages to show when those patterns match. Rules are stored in `.pi/hookify.{rule-name}.local.md` files.
...
- **Location**: `.pi/` directory in project root
- **Naming**: `.pi/hookify.{descriptive-name}.local.md`
- **Gitignore**: Add `.pi/*.local.md` to `.gitignore`
