...
- **Zero runtime risk** — Pure Markdown skill. The entire repository contains only `.md` files — no hooks, no shell scripts, no executable code, no `package.json`, no build step. Nothing runs on install or invocation beyond Pi's native Markdown skill loader.

## Installation

This skill ships with Everything Pi. No separate installation is needed when ECC is installed.
...
If you are vendoring only this skill outside the full ECC install, copy the reviewed file from the ECC repository into `/home/ryan/.pi/agent/skills/blueprint/SKILL.md`. Vendored copies do not have a git remote, so update them by re-copying the file from a reviewed ECC commit rather than running `git pull`.

## Requirements

- Pi (for `/blueprint` slash command)
