---
name: agentic-os
description: Build persistent multi-agent operating systems on Pi. Covers kernel architecture, specialist agents, slash commands, file-based memory, scheduled automation, and state management without external databases.
metadata:
  origin: ECC
---
...
Treat Pi as a persistent runtime / operating system rather than a chat session. This skill codifies the architecture used by production agentic setups: a kernel config that routes tasks to specialist agents, persistent file-based memory, scheduled automation, and a JSON/markdown data layer.

## When to Activate

- Building a multi-agent workflow inside Pi
- Setting up persistent Pi automation that survives session restarts
...
├── .pi/commands/  # Slash commands: user-facing CLI
...
| Commands (`.pi/commands/`) | User-facing slash commands (`/daily-sync`, `/outreach`) | Git-tracked |
...
`AGENTS.md` is the kernel. It acts as the COO / orchestrator. Pi reads it at session start and uses it to route work.
...
Each agent is a standalone markdown file in `agents/`. Pi loads the relevant agent file when routing a task.
...
- Test runner access
- MCP servers as configured in `.pi/mcp.json`
...
For parallel execution, use Pi's background task capability or shell scripts that invoke Pi with specific agent contexts.

## Commands and Daily Workflows

Slash commands are markdown files in `.pi/commands/`. They define reusable workflows.
...
Place command files in `.pi/commands/<command-name>.md`. Pi auto-discovers them. Users invoke them with `/<command-name>`.
...
Agentic OS tasks run on a schedule using external cron, not Pi's built-in cron (which dies when the session ends).
...
[Service]
Type=oneshot
ExecStart=/usr/local/bin/pi --cwd /path/to/project --command /daily-sync
```
...
Starting fresh every time Pi opens.
```
...
- [ ] Scheduled tasks use external cron (LaunchAgent, systemd, pm2), not Pi's session cron
