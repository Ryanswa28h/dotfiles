...
① **Run it once by hand** (forces you to state exactly "how the judge decides") → ② harden into a skill / Pi sub-agents (a main Pi loops, dispatching plan/build/judge) → ③ hang it on cron for full automation.
...
| 5 | Bloated AGENTS.md + stale memory → **the faster it loops, the more it errs** | Are the docs/memory it depends on fresh? Who maintains them? | Layered memory + periodic lint |
