...
*Use for Claude Code, Pi, Devin, AutoGPT, and any AI that takes autonomous actions. Runaway loops and scope explosion are the biggest credit killers in agentic workflows — stop conditions are not optional.*
...
**Claude Code / Pi — add Session Strategy block when relevant:**
```
## Session Strategy
[Pick one:]
...
- Compact first — Claude Code: run /compact [focus on X]. Pi: start a fresh session (new sessions are preferred for independent tasks)
```
