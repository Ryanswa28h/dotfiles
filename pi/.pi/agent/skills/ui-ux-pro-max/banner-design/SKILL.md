---
name: banner-design
description: "Design banners for social media, ads, website heroes, creative assets, and print. Multiple art direction options with AI-generated visuals. Actions: design, create, generate banner. Platforms: Facebook, Twitter/X, LinkedIn, YouTube, Instagram, Google Display, website hero, print. Styles: minimalist, gradient, bold typography, photo-based, illustrated, geometric, retro, glassmorphism, 3D, neon, duotone, editorial, collage. Uses ui-ux-pro-max skill for design intelligence."
...
1. **Create HTML/CSS banner**
   ...
   - Inject brand context via `inject-brand-context.cjs` from the `brand` skill

2. **Generate visual elements** with `ai-artist` + `ai-multimodal` skills

   **a) Search prompt inspiration**: Use the `design` skill's logo/CIP scripts or web search for banner style references.

   **b) Generate with AI** (backgrounds, patterns, hero visuals):
   Use any available Gemini-based image generation tool. Requires `google-genai` Python package and `GEMINI_API_KEY`:
   ```bash
   python3 -c "
   from google import genai
   client = genai.Client()
   image = client.models.generate_content(model='gemini-2.5-flash-image', contents='<banner visual prompt>')
   print(image.text)
   "
   ```

   **c) For complex hero visuals**, use a Pro model like `gemini-2.5-pro-image` for 4K detailed output.
...
After designing HTML banners, export each to PNG using a screenshot tool:

1. **Serve HTML files** via local server (`python3 -m http.server 8765` or similar)
2. **Screenshot each banner** at exact platform dimensions using a headless browser or CLI:
   - Use Playwright: `npx playwright screenshot --url "http://localhost:8765/banner.html" --viewport "1500,500" --output "banner.png"`
   - Or screenshot manually from the browser's DevTools
   - Ensure 2x device scale factor for crisp output
