...
After generation, **ALWAYS** ask user about HTML preview. If yes, use the `ui-ux-pro-max` skill for gallery.
...
22 art direction styles across social, ads, web, print. Uses `ui-ux-pro-max` for design intelligence, HTML/CSS for layout, and browser DevTools or Playwright for export.
...
3. **Design** — Create HTML/CSS banner, generate visuals with Gemini AI
4. **Export** — Screenshot to PNG at exact dimensions via browser DevTools or Playwright
...
Multi-platform social image design: HTML/CSS → screenshot export. Uses `ui-ux-pro-max`, `brand`, `design-system` skills.
...
3. **Ideate** — 3-5 concepts, present to user for selection
4. **Design** — Use `brand` skill → `design-system` skill → randomly invoke `ui-ux-pro-max` OR general frontend design; HTML per idea × size
5. **Export** — Playwright or browser screenshot at exact px (2x deviceScaleFactor)
...
**Related Skills:** ui-ux-pro-max, brand, design-system
