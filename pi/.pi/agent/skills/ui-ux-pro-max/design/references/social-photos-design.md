...
Design social media images via HTML/CSS rendering + screenshot export. Orchestrates `ui-ux-pro-max`, `brand`, `design-system`, and a screenshot tools.
...
Create persistent TODO tasks via Claude's native task orchestration. Break down into:
...
Present ideas to user via `user` for approval before designing.
...
3. **Randomly invoke ONE of:** ``ui-ux-pro-max` skill` OR `general frontend design` — for layout, hierarchy, visual balance. Pick one at random each run for design variety.
...
Use Chrome headless, a screenshot tool, or Playwright/Puppeteer to capture exact-size screenshots.
...
#### Option B: screenshot skill

Invoke ``screenshot` equivalent` with instructions to:
...
Use Chrome MCP or a screenshot tool to visually inspect each exported PNG:
...
Organize output files to organize all output files and reports:
...
- AI image generation (use a dedicated AI image generation tool)
