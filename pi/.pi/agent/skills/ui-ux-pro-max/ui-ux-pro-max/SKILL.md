...
The search script lives inside this skill's own directory (`scripts/search.py`), not the project directory. All commands below assume the skill directory as the working directory — cd there first or use the full path.

Skill directory:
```
~/dotfiles/pi/.pi/agent/skills/ui-ux-pro-max/ui-ux-pro-max/
```
...
# Step 4: stack guidelines
python3 scripts/search.py "suspense streaming bundle" --stack nextjs
```
