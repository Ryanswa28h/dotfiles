...
| **AskUserQuestion** / **question** | Get user input | Disambiguation, confirmation, preferences |
| **Task** / **task** | Spawn subagents | Delegating complex subtasks |
| **TaskCreate/TaskUpdate/TaskList** / **todowrite** | Track progress | Multi-step workflows with dependencies |
| **TodoRead/TodoWrite** / **todowrite** | Track progress via todo list | Most skills and agents — enables progress tracking during execution |
...
| Get user confirmation | **AskUserQuestion** / **question** | Printing and hoping |
...
# Add todowrite/TodoWrite if progress tracking is needed
```
...
```yaml
allowed-tools: Read Glob Grep question
```
...
```yaml
allowed-tools: Bash Read Write Glob Grep question task todowrite
```
...
**Agent tool principle:** Agents should have the minimum tools needed for their specific task. A read-only analysis agent should not have Write or Bash. Most agents should include progress tracking tools.
...
| No user confirmation tool on interactive skill | Skill can't get user confirmation | Add question/AskUserQuestion if any gate/confirmation exists |
| Missing progress tracking on pipeline | Can't track multi-step progress | Add task tracking tools for pipeline patterns |
