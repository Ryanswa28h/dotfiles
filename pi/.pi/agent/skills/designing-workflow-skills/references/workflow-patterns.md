...
- Task tracking coordinates multi-step execution
...
Ask user with clear options (using question/AskUserQuestion tool).
...
**Key characteristics:**
- Task tracking for state management
...
- Track: "Step A" (no dependencies)
- Track: "Step B" (blocked by Step A)
- Track: "Step C" (blocked by Step A)
- Track: "Step D" (blocked by Step B, Step C)
...
- Progress reporting provides natural status visibility

**Common mistakes:**
- Creating tasks without dependency declarations, then executing in wrong order
- Not checking progress after completing a task (missing newly unblocked work)
