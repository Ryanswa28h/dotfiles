...
Use this skill when a user needs help understanding, navigating, installing, or choosing parts of Everything Pi.
...
```bash
node scripts/install-plan.js --list-profiles
node scripts/install-plan.js --profile minimal --target pi --json
node scripts/install-apply.js --profile minimal --target pi --dry-run
```

For specific skill installs:

```bash
node scripts/install-plan.js --skills <skill-id> --target pi --json
node scripts/install-apply.js --skills <skill-id> --target pi --dry-run
```
...
- `.pi/`, `.cursor/`, `.codex/`, `.gemini/`, `.opencode/`, `.codebuddy/`, `.joycode/`, or `.qwen/`
- `hooks/hooks.json`
- install-state files
...
- `/security-scan`: inspect Pi configuration security
