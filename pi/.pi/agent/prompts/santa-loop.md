...
Run two independent reviewers (high-effort Pi Agent + external model) against the current task output. Both must return NICE before the code is pushed. If either returns NAUGHTY, fix all flagged issues, commit, and re-run fresh reviewers — up to 3 rounds.
...
#### Reviewer A: Pi Agent (always runs)

Launch a Pi Agent (subagent_type: `code-reviewer`) with the full rubric + all files under review. The prompt must include:
- The complete rubric
...
#### Reviewer B: External Model (Pi fallback only if no external CLI installed)
...
**Pi Agent fallback** (only if neither `codex` nor `gemini` is installed)
Launch a second Pi Agent (subagent_type: `code-reviewer`). Log a warning that both reviewers share the same agent family — true model diversity was not achieved but context isolation is still enforced.
...
- Reviewer A (Pi Agent, high effort) always runs — guarantees at least one strong reviewer regardless of tooling.
- Model diversity is the goal for Reviewer B. GPT-5.4 or Gemini 2.5 Pro gives true independence — different training data, different biases, different blind spots. The Pi-only fallback still provides value via context isolation but loses model diversity.
- Strongest available models are used: high-effort Pi Agent for Reviewer A, GPT-5.4 or Gemini 2.5 Pro for Reviewer B.
